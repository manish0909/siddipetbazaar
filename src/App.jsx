...
• Phone OTP verification is mandatory for listings.
...
5. ADMIN RIGHTS
• Admin may approve, reject, or remove any listing or review.
...
export default function App() {
  ...
  const googleLogin = async()=>{
    ...
    try{
      ...
      setAuthErr(e.code||"Google sign-in failed. Try again.");
    }
  ...
  const toggleFav = async(bizId)=>{
    ...
    toast(isFav?"Removed from favourites":"Saved ♥");
  };
  ...
  const sendOtp = async()=>{
    ...
    try{
      ...
      setConfirmResult(r); setOtpStep("sent"); toast("OTP sent! 📱");
    }catch(e){
  ...
  const verifyOtp = async()=>{
    ...
    catch(e){ setPhoneErr("Incorrect OTP."); setOtpStep("sent"); }
  };
  ...
  const isOwnBusiness = (biz) => user && biz?.ownerUid === user.uid;

  const submitReview = async()=>{
    if(!user){ toast("Sign in to review","err"); return; }
    if(isOwnBusiness(selected)){ toast("You cannot review your own business","err"); return; }
    if(!reviewForm.visited){ setReviewErr("Please confirm you have visited this business"); return; }
    if(!reviewForm.text.trim()){ setReviewErr("Please write your review"); return; }
    try{
  ...
  const submitReply = async(reviewId)=>{
    ...
    if(!text){ toast("Write a reply first","err"); return; }
    try{
      ...
      setShowReply(r=>({...r,[reviewId]:false}));
      toast("Reply posted ✓");
    ...
    <div style={{fontFamily:"'Outfit','Noto Sans Telugu',sans-serif",minHeight:"100vh",background:"#FFF8F2",color:"#111"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .inp:focus{border-color:#E8450A;} .inp::placeholder{color:#CCC;}
        ...
        .btn{padding:13px 22px;border-radius:12px;border:none;cursor:pointer;font-family:inherit;font-weight:600;font-size:14px;transition:all 0.2s;}
        .btn-o{background:#E8450A;color:white;} .btn-o:hover{background:#D03D09;transform:translateY(-1px);box-shadow:0 6px 20px rgba(232,69,10,0.3);}
        .btn-o:disabled{background:#EEE;color:#BBB;cursor:not-allowed;transform:none;box-shadow:none;}
        .btn-w{background:white;color:#111;border:1.5px solid #EEE;} .btn-w:hover{border-color:#CCC;}
        .btn-g{background:#F0FDF4;color:#16A34A;border:1.5px solid #BBF7D0;} .btn-g:hover{background:#DCFCE7;}
        .btn-r{background:#FEF2F2;color:#DC2626;border:1.5px solid #FECACA;} .btn-r:hover{background:#FEE2E2;}
        .btn-dark{background:#1a1208;color:white;} .btn-dark:hover{background:#2d1f0e;}
        ...
        .nav{background:rgba(255,248,242,0.92);backdrop-filter:blur(20px);border-bottom:1px solid #F5EDE8;padding:0 28px;height:62px;display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;z-index:100;}
        ...
        .star{font-size:22px;cursor:pointer;transition:transform 0.1s;} .star:hover{transform:scale(1.2);}
        .otp-inp{letter-spacing:12px;font-size:22px;font-weight:700;text-align:center;}
        .fav-btn{background:none;border:none;cursor:pointer;font-size:20px;transition:transform 0.2s;padding:4px;} .fav-btn:hover{transform:scale(1.2);}
        .dash-tab{padding:10px 20px;border:none;background:transparent;cursor:pointer;font-family:inherit;font-weight:600;font-size:14px;color:#AAA;border-bottom:2.5px solid transparent;transition:all 0.2s;}
        .dash-tab.on{color:#E8450A;border-bottom-color:#E8450A;}
        .owner-reply{background:#FFF8F2;border-left:3px solid #E8450A;border-radius:0 10px 10px 0;padding:12px 14px;margin-top:10px;}
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {user ? (
            <>
              ...
              {isAdmin&&<button className="btn btn-w" style={{fontSize:12,padding:"7px 14px",borderRadius:50}} onClick={()=>setView("admin")}>Admin</button>}
            ...
            <>
              ...
              <button className="btn btn-o" style={{fontSize:12,padding:"7px 16px",borderRadius:50}} onClick={()=>{setView("signup");setAuthErr("");setAgreed(false);setTermsError(false);}}>Join Free</button>
            </>
          )}
        </div>
      </nav>
        ...
        <>
          <div style={{background:"linear-gradient(135deg,#1a1208 0%,#2d1f0e 60%,#3d2910 100%)",padding:"70px 24px 80px",position:"relative",overflow:"hidden"}}>
            ...
            <div style={{maxWidth:680,margin:"0 auto",textAlign:"center",position:"relative"}}>
              ...
              <h1 style={{fontSize:50,fontWeight:900,letterSpacing:"-1.5px",lineHeight:1.1,color:"white",marginBottom:16}}>
                Find Every Local<br/><span style={{color:"#E8450A"}}>Business</span> Near You
              </h1>
              ...
              <div style={{position:"relative",maxWidth:480,margin:"0 auto 36px"}}>
                ...
                <input className="inp" style={{paddingLeft:50,borderRadius:50,fontSize:15,padding:"16px 20px 16px 50px",background:"rgba(255,255,255,0.1)",border:"1px solid rgba(255,255,255,0.15)",color:"white"}}
                  placeholder={lang==="en"?"Search by name, tag, area...":"వ్యాపారాలు వెతకండి..."}
          ...
          <div style={{maxWidth:780,margin:"0 auto",padding:"36px 20px 60px"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:20}}>
              <div>
                <div style={{fontWeight:800,fontSize:19,letterSpacing:"-0.3px"}}>
                  {search ? `Results for "${search}"` : activeCat==="All"?"All Businesses":activeCat}
                </div>
                <div style={{color:"#BBB",fontSize:13,marginTop:3}}>{filtered.length} listings</div>
              </div>
              {!user&&<button className="btn btn-o" style={{fontSize:12,padding:"8px 18px",borderRadius:50}} onClick={()=>setView("signup")}>+ List Business</button>}
            </div>

            {dbLoading ? (
              <div style={{padding:"80px 0",textAlign:"center"}}><div className="spin" style={{marginBottom:16}}></div><div style={{color:"#CCC",fontSize:13}}>Loading...</div></div>
            ) : filtered.length===0 ? (
              <div style={{textAlign:"center",padding:"80px 0",color:"#CCC"}}>
                <div style={{fontSize:48,marginBottom:12}}>🔍</div>
                <div style={{fontWeight:700,color:"#999",fontSize:16}}>{search?"No results found":"No businesses yet"}</div>
                <div style={{fontSize:13,marginTop:6}}>{search?"Try a different search term":"Be the first to add one!"}</div>
              </div>
            ) : (
              <div className="grid">
                {filtered.map(b=>(
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:12}}>
                        {b.tags.slice(0,3).map(t=>(
                          <span key={t} className="tag"
                            style={{background:search&&t.toLowerCase().includes(search.toLowerCase())?"#E8450A":"#FFF0EA",
                                    color:search&&t.toLowerCase().includes(search.toLowerCase())?"white":"#E8450A"}}>
                            {t}
                          </span>
                        ))}
                      </div>
          ...
          <div style={{background:"white",borderRadius:24,padding:40,width:"100%",maxWidth:400,border:"1.5px solid #F5EDE8",boxShadow:"0 20px 60px rgba(232,69,10,0.07)"}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              <input className="inp" type="password" placeholder="Password" value={authForm.password}
                onChange={e=>setAuthForm(f=>({...f,password:e.target.value}))} onKeyDown={e=>e.key==="Enter"&&login()}/>
              ...
              <div style={{textAlign:"center",color:"#AAA",fontSize:13}}>
                No account? <span style={{color:"#E8450A",cursor:"pointer",fontWeight:600}} onClick={()=>{setView("signup");setAgreed(false);setTermsError(false);}}>Sign up free</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {view==="signup"&&(
        <div style={{minHeight:"85vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
          <div style={{background:"white",borderRadius:24,padding:40,width:"100%",maxWidth:420,border:"1.5px solid #F5EDE8",boxShadow:"0 20px 60px rgba(232,69,10,0.07)"}}>
            <div style={{marginBottom:28}}>
              <div style={{fontWeight:800,fontSize:24,letterSpacing:"-0.5px",marginBottom:6}}>Join Siddipet Bazaar</div>
              <div style={{color:"#AAA",fontSize:14}}>One account — browse, save, list & review</div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{display:"flex",alignItems:"flex-start",gap:12,cursor:"pointer"}} onClick={()=>agreed?setAgreed(false):setShowTerms(true)}>
                  ...
                  <div style={{fontSize:13,color:"#666",lineHeight:1.5}}>
                    ...
                    <span style={{color:"#E8450A",fontWeight:600,textDecoration:"underline",cursor:"pointer"}}
                      onClick={e=>{e.stopPropagation();setShowTerms(true);}}>Terms & Conditions</span>
                  </div>
                </div>
                {termsError&&<div style={{color:"#DC2626",fontSize:12,marginTop:8,marginLeft:32}}>Please accept terms to continue</div>}
              </div>

              <button className="btn btn-w" onClick={googleLogin} disabled={authLoading||!agreed}
                style={{width:"100%",padding:13,display:"flex",alignItems:"center",justifyContent:"center",gap:10,fontSize:14,borderRadius:12,opacity:agreed?1:0.4,cursor:agreed?"pointer":"not-allowed"}}>
                <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" width="17" style={{opacity:agreed?1:0.4}}/>
                Continue with Google
              </button>

              <div className="divider">or sign up with email</div>
              <input className="inp" placeholder="Your name" value={authForm.name} onChange={e=>setAuthForm(f=>({...f,name:e.target.value}))}/>
              <input className="inp" type="email" placeholder="Email address" value={authForm.email} onChange={e=>setAuthForm(f=>({...f,email:e.target.value}))}/>
              ...
              <button className="btn btn-o" style={{width:"100%",padding:14,opacity:agreed?1:0.4}} onClick={signup} disabled={authLoading||!agreed}>
                {authLoading?"Creating account...":"Create Free Account →"}
              </button>
              <div style={{textAlign:"center",color:"#AAA",fontSize:13}}>
                Have an account? <span style={{color:"#E8450A",cursor:"pointer",fontWeight:600}} onClick={()=>setView("login")}>Sign in</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {view==="dashboard"&&user&&(
        <div style={{maxWidth:700,margin:"0 auto",padding:"40px 24px"}}>
          {/* Profile */}
          <div style={{background:"linear-gradient(135deg,#1a1208,#3d2910)",borderRadius:20,padding:28,marginBottom:4,color:"white"}}>
            <div style={{fontSize:11,color:"rgba(232,69,10,0.8)",letterSpacing:1,textTransform:"uppercase",marginBottom:8}}>My Account</div>
            <div style={{fontWeight:800,fontSize:22,marginBottom:4}}>👋 {userProfile?.name||user.name}</div>
            ...
            <div style={{display:"flex",gap:28,marginTop:20,paddingTop:20,borderTop:"1px solid rgba(255,255,255,0.1)"}}>
              {[["❤️",favourites.length,"Saved"],["🏪",myList.length,"Listings"],["✅",myList.filter(b=>b.status==="approved").length,"Live"]].map(([ic,v,l])=>(
          ...
          {dashTab==="favourites"&&(
            favList.length===0?(
              <div style={{background:"white",borderRadius:16,padding:32,textAlign:"center",color:"#CCC",border:"1.5px solid #F5EDE8",fontSize:14}}>
                No saved businesses — tap 🤍 on any listing!
              </div>
            ):(
              <div className="grid">
                {favList.map(b=>(
                  <div key={b.id} className="card" onClick={()=>{ setSelected(b); loadReviews(b.id); setShowReviewForm(false); }}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                      <div style={{fontSize:30,width:48,height:48,background:"#FFF0EA",borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{b.icon||"🏪"}</div>
          ...
          {/* Tab: Add Business */}
          {dashTab==="add"&&(
            <div style={{background:"white",borderRadius:20,padding:28,border:"1.5px solid #F5EDE8"}}>
              <div style={{fontWeight:700,fontSize:17,marginBottom:22}}>Submit New Listing</div>
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    <div style={{position:"relative",flex:1}}>
                      ...
                      <input className="inp" style={{paddingLeft:46,borderColor:otpStep==="verified"?"#BBF7D0":phoneErr?"#FECACA":"#EEE"}}
                        ...
                        onChange={e=>{ const v=e.target.value.replace(/\D/g,""); setAddForm(f=>({...f,phone:v})); setPhoneErr(""); if(otpStep!=="idle"){setOtpStep("idle");setOtp("");} }}/>
                    </div>
                    ...
                    {otpStep==="verified"&&<button className="btn btn-g" disabled style={{whiteSpace:"nowrap"}}>✓</button>}
                  </div>
                  {otpStep==="sent"&&(
                ...
                <button className="btn btn-o" style={{alignSelf:"flex-start",padding:"12px 28px"}} onClick={submitListing}>
                  Submit for Review →
                </button>
              </div>
            </div>
      ...
      {view==="admin"&&isAdmin&&(
        <div style={{maxWidth:740,margin:"0 auto",padding:"40px 24px"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <div style={{fontWeight:700,fontSize:17}}>Pending Approvals</div>
            <button className="btn btn-w" style={{fontSize:12,padding:"7px 16px",borderRadius:50}} onClick={loadBiz}>↺ Refresh</button>
          </div>
          {pending.length===0?(
            <div style={{background:"white",borderRadius:16,padding:28,textAlign:"center",color:"#CCC",border:"1.5px solid #F5EDE8",marginBottom:24,fontSize:14}}>All caught up 🎉</div>
          ):(
      ...
      {selected&&(
        <div className="overlay" onClick={()=>{ setSelected(null); setShowReviewForm(false); }}>
          <div className="sheet" onClick={e=>e.stopPropagation()}>
            ...
            {selected.tags?.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:20}}>{selected.tags.map(t=><span key={t} className="tag">{t}</span>)}</div>}
            ...
            <div style={{borderTop:"1px solid #F5EDE8",paddingTop:20}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
                ...
                {user&&!isOwnBusiness(selected)&&!showReviewForm&&(
                  <button className="btn btn-o" style={{fontSize:12,padding:"7px 16px",borderRadius:50}} onClick={()=>setShowReviewForm(true)}>
                    + Write Review
                  </button>
                )}
                {user&&isOwnBusiness(selected)&&(
                  <span style={{fontSize:12,color:"#AAA",fontStyle:"italic"}}>Your business</span>
                )}
                {!user&&(
              ...
              {/* Review form */}
              {showReviewForm&&user&&!isOwnBusiness(selected)&&(
                <div style={{background:"#FFF8F2",borderRadius:16,padding:20,marginBottom:16,border:"1.5px solid #F5EDE8"}}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:12}}>
                    <button className="btn btn-o" style={{flex:1,padding:11}} onClick={submitReview}>Submit</button>
              ...
              {(reviews[selected.id]||[]).length===0?(
                <div style={{textAlign:"center",color:"#CCC",padding:"20px 0",fontSize:13}}>No reviews yet — be the first!</div>
              ):(
                <div style={{display:"flex",flexDirection:"column",gap:14}}>
                  {(reviews[selected.id]||[]).map(r=>(
                    <div key={r.id} style={{background:"#FFF8F2",borderRadius:14,padding:16,border:"1px solid #F5EDE8"}}>
                      ...
                      <div style={{fontSize:11,color:"#CCC",marginTop:6}}>{new Date(r.createdAt).toLocaleDateString("en-IN")}</div>

                      {/* Owner reply display */}
                      {r.ownerReply&&(
                        <div className="owner-reply">
                          <div style={{fontSize:11,fontWeight:700,color:"#E8450A",marginBottom:4}}>🏪 Owner Reply</div>
                          <div style={{fontSize:13,color:"#555",lineHeight:1.6}}>{r.ownerReply.text}</div>
                          <div style={{fontSize:10,color:"#CCC",marginTop:4}}>{new Date(r.ownerReply.repliedAt).toLocaleDateString("en-IN")}</div>
                        </div>
                      )}

                      {/* Owner reply form — only for the business owner, only if no reply yet */}
                      {isOwnBusiness(selected)&&!r.ownerReply&&(
                        <div style={{marginTop:10}}>
                          {showReply[r.id]?(
                            <div>
                              <textarea className="inp" placeholder="Write your reply..." rows={2}
                                style={{resize:"none",fontSize:13,marginBottom:8}}
                                value={replyText[r.id]||""}
                                onChange={e=>setReplyText(t=>({...t,[r.id]:e.target.value}))}/>
                              <div style={{display:"flex",gap:8}}>
                                <button className="btn btn-o" style={{fontSize:12,padding:"7px 16px"}} onClick={()=>submitReply(r.id)}>Post Reply</button>
                                <button className="btn btn-w" style={{fontSize:12,padding:"7px 14px"}} onClick={()=>setShowReply(s=>({...s,[r.id]:false}))}>Cancel</button>
                              </div>
                            </div>
                          ):(
                            <button className="btn btn-w" style={{fontSize:11,padding:"6px 14px",marginTop:4}} onClick={()=>setShowReply(s=>({...s,[r.id]:true}))}>
                              ↩ Reply as Owner
                            </button>
                          )}
                        </div>
